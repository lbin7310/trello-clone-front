...
class Card extends Component {
  ...
  onCreateCard = () => {
    const { newCardTitle, cardToggle, containerId } = this.state;
    if (newCardTitle !== '') {
  ...
  onIsActive = (e, active) => {
    isActiveCard(e.target.id, active).then(res => {
      this.setState({
        cards: res.data,
      });
    });
      ...
      <div className="cards">
        ...
        >
          ...
          <div className="card__Add-card-buttons">
            <button onClick={onCreateCard}>Add card</button>
            <button onClick={onCardToggle}>Cancel</button>
          </div>
        </div>
        <div className="card__buttons">
          <button
            onClick={onCompletedCards}
            style={{ display: toggle ? '' : 'none' }}
          >
            Completed Cards
          </button>
          <button
            onClick={onOngoingCards}
            style={{ display: !toggle ? '' : 'none' }}
          >
            Ongoing Cards
          </button>
    ...
    return (
      <div>
        <i
          ...
          id={cardid}
          onClick={e => onIsActive(e, isActive)}
        />
      </div>
    );
  }
